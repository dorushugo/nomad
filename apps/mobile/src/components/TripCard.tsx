...
const DESTINATION_EMOJIS: Record<string, string> = {
  japon: "🗼", japan: "🗼", tokyo: "🗼", paris: "🗼",
  france: "🇫🇷", italie: "🇮🇹", italy: "🇮🇹", espagne: "🇪🇸", spain: "🇪🇸",
  bali: "🌴", thailande: "🏝️", thailand: "🏝️", usa: "🇺🇸", maroc: "🕌",
  grece: "🏛️", greece: "🏛️", portugal: "🇵🇹", mexique: "🇲🇽", mexico: "🇲🇽",
};
...
function getStatusSummary(trip: Trip, status: TripTimingStatus, dayCount: number, daysUntil: number | null, c: ThemeColors) {
  switch (status) {
    case "current":
      return {
        label: "Aujourd'hui", value: `Jour ${getCurrentDay(trip)} / ${dayCount}`,
        backgroundColor: "rgba(0, 138, 5, 0.08)", borderColor: "rgba(0, 138, 5, 0.18)",
        labelColor: c.green, valueColor: c.black,
      };
    case "past":
      return {
        label: "Statut", value: "Terminé",
        backgroundColor: c.grayLight, borderColor: c.grayBorder,
        labelColor: c.gray, valueColor: c.black,
      };
    case "future":
    default:
      return {
        label: "Départ", value: formatCountdown(daysUntil) || "À définir",
        backgroundColor: c.roseLight, borderColor: c.roseMuted,
        labelColor: c.rose, valueColor: c.black,
      };
  }
}
  ...
  return (
    ...
    >
      ...
      <View style={styles.body}>
        ...
        <View style={[styles.summaryRow, !trip.description ? styles.summaryRowNoDescription : null]}>
          <View style={[styles.summaryCard, { backgroundColor: statusSummary.backgroundColor, borderColor: statusSummary.borderColor }]}>
            <Text style={[styles.summaryLabel, { color: statusSummary.labelColor }]}>{statusSummary.label}</Text>
            <Text style={[styles.summaryValue, { color: statusSummary.valueColor }]}>{statusSummary.value}</Text>
          </View>
          <View style={styles.summaryCard}>
            <Text style={styles.summaryLabel}>Durée</Text>
            <Text style={styles.summaryValue}>{dayCount} jour{dayCount > 1 ? "s" : ""}</Text>
          </View>
        </View>
...
const makeStyles = (c: ThemeColors) =>
  StyleSheet.create({
    ...
    emojiContainer: {
      ...
      backgroundColor: c.grayLight, alignItems: "center", justifyContent: "center", marginRight: spacing.md,
    },
    emoji: { fontSize: 24 },
    headerContent: { flex: 1 },
    titleRow: { flexDirection: "row", alignItems: "center", marginBottom: 2 },
    statusBadge: { paddingHorizontal: spacing.sm + 2, paddingVertical: spacing.xs + 2, borderRadius: radius.full, marginLeft: spacing.sm },
    statusBadgeText: { fontFamily: fonts.semiBold, fontSize: fontSize.xs, letterSpacing: 0.2 },
    destination: { fontFamily: fonts.semiBold, fontSize: fontSize.md, color: c.rose, letterSpacing: -0.1 },
    title: { fontFamily: fonts.bold, fontSize: fontSize.xl, color: c.black, letterSpacing: -0.3, flex: 1, flexShrink: 1 },
    dateRow: { flexDirection: "row", alignItems: "center", gap: spacing.xs + 2, marginTop: spacing.md },
    dates: { fontFamily: fonts.medium, fontSize: fontSize.sm, color: c.gray },
    description: { fontFamily: fonts.regular, fontSize: fontSize.sm, color: c.gray, lineHeight: 20, marginBottom: spacing.md, marginTop: spacing.sm + 2 },
    summaryRow: { flexDirection: "row", gap: spacing.sm },
    summaryRowNoDescription: { marginTop: spacing.md },
    summaryCard: {
      flex: 1, backgroundColor: c.grayLight, borderWidth: 1, borderColor: c.grayBorder,
      borderRadius: radius.lg, paddingHorizontal: spacing.md, paddingVertical: spacing.md,
      minHeight: 78, justifyContent: "space-between",
    },
    summaryLabel: { fontFamily: fonts.semiBold, fontSize: fontSize.xs, color: c.gray, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: spacing.sm },
    summaryValue: { fontFamily: fonts.bold, fontSize: fontSize.lg, color: c.black, letterSpacing: -0.2 },
    footer: { flexDirection: "row", marginTop: spacing.md },
    pill: { flexDirection: "row", alignItems: "center", gap: spacing.sm, paddingHorizontal: spacing.md, paddingVertical: spacing.sm + 2, borderRadius: radius.full },
    pillLabel: { fontFamily: fonts.semiBold, fontSize: fontSize.xs, color: c.gray, textTransform: "uppercase", letterSpacing: 0.8 },
